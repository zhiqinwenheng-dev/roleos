...
const sections: DocSection[] = [
  {
    ...
    subtitle: { zh: '先理解平台，再开始使用', en: 'Understand before you execute' },
  ...
  },
  {
    ...
    subtitle: { zh: 'Cloud 使用与引导流程', en: 'Cloud onboarding and usage' },
  ...
  const content = useMemo(() => {
    if (active === 'overview') {
      return {
        ...
        summary: isZh
          ? '文档不是概念展示，而是让用户能在最短路径内完成“理解 -> 部署 -> 运行 -> 运营”。'
        ...
        blocks: [
          ...
          },
          {
            ...
            points: isZh
              ? ['1. 先读对象标准', '2. 再读 RS 或 Rc 使用路径', '3. 最后看运营与计费']
        ...
        blocks: [
          {
            ...
            points: isZh
              ? ['准备服务器或本地机器', '配置大模型 API 凭证', '配置飞书/机器人凭证']
    ...
    if (active === 'rc') {
      return {
        ...
        summary: isZh
          ? '注册即用，适合快速试用、持续运行与商业转化。'
          : 'Sign up and use immediately for fast trial and commercial conversion.',
        blocks: [
          {
          ...
          },
          {
            ...
            points: isZh
              ? ['one channel（Feishu）', 'one starter Role / Kit / Team', 'one model policy']
              : ['one channel (Feishu)', 'one starter Role / Kit / Team', 'one model policy']
          }
        ]
      };
    ...
    if (active === 'standards') {
      return {
        ...
        summary: isZh
          ? '这是 RS 与 Rc 共用的核心语义层，也是后续市场与插件扩展的基础。'
          : 'The shared semantic layer for both RS and Rc and the base for future marketplace expansion.',
        blocks: [
          {
            heading: 'Role',
            points: isZh ? ['定义目标、输入输出、约束', '对应谁来完成工作'] : ['Defines goals, I/O, constraints', 'Represents who does the work']
          },
          {
            heading: 'Kit',
            points: isZh ? ['定义依赖资产、安装目标、模型策略', '对应如何启动工作'] : ['Defines assets, install target, model policy', 'Represents how work starts']
          },
          {
            heading: 'Team',
            points: isZh ? ['定义角色协作、交接和检查点', '对应如何完成复杂协作'] : ['Defines collaboration, handoff, checkpoints', 'Represents coordinated execution']
          }
        ]
      };
    }
      ...
      <div className="max-w-5xl mx-auto">
        <section className="text-center mb-10">
          ...
          <h1 className="mt-4 text-4xl md:text-5xl font-bold text-black">
            {isZh ? '文档中心（RS / Rc 统一）' : 'Unified Docs for RS / Rc'}
          </h1>
          <p className="mt-4 text-black/55 max-w-3xl mx-auto leading-relaxed">
            {isZh
              ? '文档结构采用“先认知、再执行”的路径：先理解对象标准，再进入 RS 或 Rc 的落地流程。'
              : 'The docs follow an understanding-first path: learn the object standard, then execute with RS or Rc.'}
          </p>
        </section>
        ...
        <section className="mt-10 text-center">
          <p className="text-sm text-black/45">
            {isZh
              ? '后续将继续补充：行业模板、案例库、迁移指南、开放 API 示例。'
