...
export default function Market() {
  ...
  async function loadCatalog() {
    ...
    try {
      ...
      setErrorText(error instanceof Error ? error.message : isZh ? "获取目录失败" : "Failed to fetch catalog");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="pt-32 pb-24 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
        <div>
          ...
          <p className="text-xl text-black/50">
            {isZh ? `RS 与 RC 共用同一份目录。当前条目数：${totalEntries}` : `RS and RC share one catalog. Entries: ${totalEntries}`}
          </p>
        </div>
        <button
        ...
        >
          ...
          <span>{isZh ? "刷新目录" : "Refresh Catalog"}</span>
        </button>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6 mb-12">
        ...
        >
          <div className="flex items-center space-x-3 mb-4">
            ...
            <h3 className="font-bold">{isZh ? "RS 商品包" : "RS Packages"}</h3>
          </div>
          <ul className="space-y-2 text-sm text-black/70">
            ...
            {!catalog?.bPackages?.length && <li className="text-black/40">{isZh ? "暂无数据" : "No data"}</li>}
          </ul>
        </motion.div>
        ...
        >
          <div className="flex items-center space-x-3 mb-4">
            ...
            <h3 className="font-bold">{isZh ? "RC 订阅计划" : "RC Plans"}</h3>
          </div>
          <ul className="space-y-2 text-sm text-black/70">
            ...
            {!catalog?.cPlans?.length && <li className="text-black/40">{isZh ? "暂无数据" : "No data"}</li>}
          </ul>
        </motion.div>
        ...
        >
          <div className="flex items-center space-x-3 mb-4">
            ...
            <h3 className="font-bold">{isZh ? "计费模式" : "Billing Modes"}</h3>
          </div>
          <ul className="space-y-2 text-sm text-black/70">
            ...
            {!catalog?.billingModes?.length && <li className="text-black/40">{isZh ? "暂无数据" : "No data"}</li>}
          </ul>
        </motion.div>
      </div>
      ...
      <div className="rounded-2xl border border-black/5 bg-white p-6 shadow-sm">
        ...
        <pre className="p-4 bg-zinc-950 text-zinc-100 rounded-xl text-xs overflow-auto min-h-[220px]">
          {catalog ? JSON.stringify(catalog, null, 2) : isZh ? "等待加载..." : "Loading..."}
        </pre>
      </div>
    </div>
  );
